...
int main(int argc, char* argv[])
{
    ...
    for(int i = 1; i < argc; i++) {
        if(strcmp(argv[i], "-id") == 0){
            ...
            LOGI("Usage: SimpleView_Triggermode30 [-h]");
            return 0;
        }
    }
    ...
    ASSERT_OK( selectDevice(TY_INTERFACE_USB, ID, IP, 1, selected) );//Only USB cam support this sample.
    ...
    LOGD("=== Set trigger to trig mode 30");
    TY_TRIGGER_PARAM_EX trigger;
    trigger.mode = TY_TRIGGER_MODE_PER_PASS2;
    ...
    while(!exit_main) {
        ...
        if (err == TY_STATUS_OK) {
            ...
            for (int32_t i=0; i<frame.validCount; i++) {
                LOGD("    [%d]%4d*%4d", i,
                    frame.image[i].width, frame.image[i].height);
            }
